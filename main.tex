...
stronger. Finally, we translate the measurement into a deployment lever: in
a controlled test, gating a second ``review/verify'' model pass---the
costliest optional stage in a typical agent pipeline---cuts latency
2.4$\times$ with \emph{no} measurable quality loss under an order-swapped LLM
judge, showing the prefill finding pays off in practice. We release all
harnesses and raw data.
\end{abstract}
