% =====================================================================
%  HotMobile 2027 version (ACM sigconf). Body content identical to the
%  arXiv article version; only the front-matter/format differ. Camera-ready
%  must use HotMobile's 10pt-amended acmart template (see README).
% =====================================================================
...
% acmart already provides amsmath, amssymb, graphicx, hyperref, natbib, xcolor

\settopmatter{printacmref=false}
\renewcommand\footnotetextcopyrightpermission[1]{}
\pagestyle{plain}
\setcopyright{none}
\emergencystretch=2em   % absorb \texttt overfull lines at narrow column edges

\begin{document}

\title{Prefill Is the Bottleneck: An Empirical Study of CPU-Only
On-Device LLM Latency and One Lever That Exploits It}

\author{Bapista Khan}
\affiliation{%
  \institution{Collab-Foundry / NeuronAI}
  \country{}}
\email{bapistakhan@gmail.com}
\renewcommand{\shortauthors}{Bapista Khan}

\begin{abstract}
On-device and mobile AI increasingly run large language models (LLMs) on
commodity CPUs with no GPU---for privacy, offline operation, and cost---yet
latency intuition remains grounded in GPU serving, where autoregressive
\emph{decode} dominates. We instrument end-to-end inference of a quantized 7B
model (Qwen2.5-7B, Q4) on a single CPU-only machine (AMD Ryzen~7 8845HS, 8
cores / 16 threads, 14~GiB) and sweep prompt length from 55 to 2{,}336 tokens
(5 repetitions per point). \emph{Prefill}---reading the prompt---grows from
38\% of end-to-end latency at 55 tokens to \textbf{95\%} at 2{,}336 tokens,
crossing 90\% for prompts beyond $\sim$1{,}300 tokens. Prefill time scales with
prompt length while decode stays roughly flat ($\sim$11--12 tok/s), so
end-to-end latency on CPU is governed by \emph{context length}, not output
length---inverting the GPU-era assumption. The effect is not model-specific
(Llama-3.1-8B and Gemma2-9B sit at 95--96\% prefill at $\sim$1{,}300 tokens),
and prefill \emph{speed} is non-monotonic in quantization: the 8-bit build
prefills fastest and the intermediate K-quant slowest, inverting the
``quantize-down-for-speed'' heuristic on CPU. Both effects reproduce on a
second CPU vendor (Intel Comet Lake), where prefill dominance is if anything
stronger. Finally, we translate the measurement into a deployment lever: in a
controlled test, gating a second ``review/verify'' model pass---the costliest
optional stage in a typical agent pipeline---cuts latency 2.4$\times$ with
\emph{no} measurable quality loss under an order-swapped LLM judge. We release
all harnesses and raw data.
\end{abstract}

\keywords{on-device inference, edge AI, mobile systems, LLM latency, prefill,
CPU inference, quantization, effort routing}
...
On-device and mobile LLM deployment---driven by privacy, data governance,
intermittent connectivity, and cost---has made CPU-only inference a real
operating regime on phones, mini-PCs, and edge boxes with no GPU offload.
The NeuronAI system this study measures runs a fleet of 7--9B quantized
models entirely on CPU on a 14~GiB mini-PC-class device.
...
of decode, so decode dominates and optimization targets it. \emph{On the
CPU-only hardware that on-device and mobile inference actually run on, is
that still true?}
...
cross-family spot-check at 7B--9B (Section~\ref{sec:results-size}), a
Q4/Q5/Q8 quantization sweep (Section~\ref{sec:results-quant}), and a
cross-vendor replication on a second CPU (Intel;
Section~\ref{sec:results-intel}). We do not study batched multi-user serving,
GPU offload, or non-x86 (ARM) hardware; those are future work
...
\subsection{Generalization to a second CPU vendor}
\label{sec:results-intel}
To test whether these findings are specific to AMD/Zen~4, we repeat the
prompt-length and quantization sweeps on a machine of a different vendor and
microarchitecture: an \textbf{Intel Core i7-10700T} (Comet Lake, 8C/16T,
35~W TDP, 2020), CPU-only, same Ollama/llama.cpp runtime and models
($N{=}5$). Both findings carry over.

\emph{Prefill still dominates---more so.} On the slower Intel part the
prefill fraction is \emph{higher} at every prompt length---51\% at 121
tokens, 85\% at 453, 93\% at 1{,}403, and 96\% at 2{,}439---than on the
Ryzen, consistent with prefill's compute-bound cost growing faster than
decode as the CPU slows. Prefill dominance is thus not a property of one
vendor; if anything, weaker CPUs are \emph{more} prefill-bound.

\emph{The non-monotonic quant ordering reproduces.} At a fixed
$\sim$1{,}400-token prompt, Q8\_0 again prefills \emph{fastest} and
Q5\_K\_M \emph{slowest} (Table~\ref{tab:intel})---the same ordering as on
AMD. ``Q8 is fastest on CPU'' is therefore a property of the dequantization
scheme, not of one microarchitecture. Absolute prefill throughput is
$\sim$2.7$\times$ lower than the Ryzen (16.8 vs.\ 46.3 tok/s at Q4), as
expected for a 2020 low-power part.

\begin{table}[t]
  \centering
  \caption{Second CPU vendor: Intel Core i7-10700T (Comet Lake), CPU-only,
  Qwen2.5-7B-Instruct, $\sim$1{,}400-token prompt, mean$\pm$std over 5 runs.
  Both prefill dominance and the non-monotonic quant ordering reproduce.}
  \label{tab:intel}
  \small
  \begin{tabular}{lrrr}
    \toprule
    ...
    \midrule
    Q4\_K\_M & 83.54$\pm$0.46  & 16.8 & 93 \\
    Q5\_K\_M & 105.23$\pm$1.67 & 13.3 & 98 \\
    Q8\_0    & 71.77$\pm$0.53  & 19.5 & 95 \\
    \bottomrule
  \end{tabular}
\end{table}
...
(Ollama/llama.cpp); the cross-family check (Table~\ref{tab:modelsize}) spans
7B--9B, the quantization sweep (Table~\ref{tab:quant}) spans Q4/Q5/Q8, and we
replicate on two x86 CPU vendors (AMD Zen~4 and Intel Comet Lake,
Table~\ref{tab:intel}). We report 5 repetitions per point (mean$\pm$std); the
main remaining extensions are \emph{non-x86} (ARM/mobile) hardware and
batched serving. We cap
...
generated. The prefill-dominance and quant findings reproduce across two x86
CPU vendors (AMD and Intel). Future work: non-x86 (ARM/mobile) hardware, a
broader quality evaluation of the routing lever, and direct evaluation of
...
\texttt{quant\_sweep.py}, \texttt{review\_pass\_experiment.py}, and
\texttt{cross\_hw\_sweep.py} for the second machine) and raw per-run
JSON/JSONL are released at
